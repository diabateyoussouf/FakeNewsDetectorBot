package com.fakenewsdetectorbot.fakenewbot.telegram;
...
public class TelegramBot extends TelegramLongPollingBot {
    ...
    private void sendTextMessage(long chatId, String text) throws TelegramApiException {
        ...
        if (text.length() > 4000) {
            truncatedText = text.substring(0, 4000) + "\n\n [Message tronqué pour respecter les limites Telegram]";
        }

        SendMessage sendMessage = new SendMessage(String.valueOf(chatId), truncatedText);
        try {
            execute(sendMessage);
        } catch (TelegramApiException e) {
            throw new RuntimeException(e);
